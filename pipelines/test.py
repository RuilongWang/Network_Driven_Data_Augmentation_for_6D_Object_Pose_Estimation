...
def test(cfg, cnn=None, create_cnn=None, output_images=True, output_stats=True):
    ...
    scale = data.read_cfg_float(cfg, 'train', 'image_scale', default=0.25)
    ...
    for model in model_to_dataset.keys():
            ...
            for region_ind in range(1, num_regions + 1, 1):
                ...
                if region_area > 32:
                    ...
                    if bbox_iou > 0.5:
                        ...
                        corr_region = cv2.resize(corr_region.astype(np.uint8), None, fx=1/scale, fy=1/scale,
        ...
        if output_stats:
            ...
            df.to_csv(log_dir + 'stats_{}.csv'.format(model), mode='a')
